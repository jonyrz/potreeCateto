...
tree.on("uncheck_node.jstree", (e, data) => {
	...
	if(object && object.Folder != "Folder"){
		object.visible = false;
	}
	else{
		for(let child of data.node.children_d){
			let datachild = $.jstree.reference("pointclouds").get_node(child);
			datachild.data.visible = false;
		}
	}
})
...
initSidebar = (viewer) => {
	...
	function initToolbar () {
		...
		// DISTANCE
		elToolbar.append(createToolIcon(
			Potree.resourcePath + '/icons/polyline.svg',
			'[title]Polyline',
			function () {
				$('#menu_measurements').next().slideDown();
				let measurement = measuringTool.startInsertion({
	...
	function initScene(){
		...
		tree.jstree({
			...
			"checkbox" : {
				...
				//CHECKBOX STATE (REPLACE FOR TRUE)
				"three_state": true,
				"whole_node": false,
				"tie_selection": false,
			},
		...
		tree.on("uncheck_node.jstree", function(e, data){
			...
			if(object && object.Folder != "Folder"){
				...
				//LOOP PARENTS & CHILDREN NODES TO CASCADE
				for(let child of data.node.children_d){
					let datachild = $.jstree.reference("pointclouds").get_node(child);
					datachild.data.visible = false;
				}
...
class PolylinePanel extends MeasurePanel{
	constructor(viewer, measurement, propertiesPanel){
		super(viewer, measurement, propertiesPanel);
